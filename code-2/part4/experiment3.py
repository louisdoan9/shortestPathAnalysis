...
class StationInfo:
    def __init__(self):
        self.stations = {}
        with open("../csv_files/london_stations.csv", 'r') as csvfile:
...
def add_stations_to_graph(graph):
    with open('../csv_files/london_stations.csv', newline='', encoding='utf-8') as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
            if (row[0] != "id"):
...
def add_stations_connections_to_graph(graph):
    with open('../csv_files/london_connections.csv', newline='', encoding='utf-8') as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
            if (row[0] != 'station1'):
...
def experiment_refactored():
    ...
    exp1 = stationsOrdered[1]
    spf = ShortPathFinder()
    spf.set_graph(x)
    spf.set_algorithm(Dijkstra)
    return spf.calc_short_path(exp1[0], exp1[1])

print(experiment_refactored())
